...
import sys
from .gameweek import find_prelim_bonus
...
def fixtureRunner(get_gw_fixture, get_data_bootstrap, curr_gw):
    print("\nGameweek:", curr_gw)
    print()
    ...
    if started:
        print_started(started, team_list, get_data_bootstrap, player_cache)
        print()
...
def print_upcoming(upcoming, team_list, get_data_bootstrap):
    ...
    for g in range(len(upcoming)):
        print("\n\n" + str(g + global_cnt + 1) + ".", end="")

        print('{:<5} {:>15}'.format(
            "", team_list[upcoming[g]["team_h"]]), end="")
...
def print_started(started, team_list, get_data_bootstrap, player_cache):
    global global_cnt
    print("\nStarted:")
    for g in range(len(started)):
        ...
        print('{:<5} {:>15}'.format(
            "", team_list[started[g]["team_h"]]), end="")
        print('{:<12} {} - {}'.format("",
                                      started[g]["team_h_score"], started[g]["team_a_score"]), end="")
        print('{:<12} {:18} ({} mins)'.format(
            "", team_list[started[g]["team_a"]], started[g]["minutes"]))

        for k in [0, 2, 1, 4, 6, 9]:
